...
var WebSnippet = /** @class */ (function () {
    function WebSnippet(client) {
        ...
        var clientSnippetConnectionString = client.config.webSnippetConnectionString;
        var defaultIkey = client.config.instrumentationKey;
        if (!!clientSnippetConnectionString) {
            var clientSnippetIkey = this._getWebSnippetIkey(client.config.webSnippetConnectionString);
            defaultIkey = clientSnippetIkey;
        }
        WebSnippet._snippet = applicationinsights_web_snippet_1.webSnippet.replace("INSTRUMENTATION_KEY", defaultIkey);
        this._statsbeat = client.getStatsbeat();
    }
    WebSnippet.prototype.enable = function (isEnabled, webSnippetConnectionString) {
        this._isEnabled = isEnabled;
        if (!!webSnippetConnectionString) {
            var iKey = this._getWebSnippetIkey(webSnippetConnectionString);
            WebSnippet._snippet = applicationinsights_web_snippet_1.webSnippet.replace("INSTRUMENTATION_KEY", iKey);
        }
